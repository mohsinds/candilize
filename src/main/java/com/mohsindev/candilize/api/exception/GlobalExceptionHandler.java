...
/**
 * Global controller advice that centralizes exception handling for all REST controllers.
 * Converts exceptions into consistent JSON error responses (ErrorResponse) with appropriate
 * HTTP status codes. Logs handled exceptions for debugging and audit.
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleUnsupportedExchangeException(
            UnsupportedExchangeException ex, HttpServletRequest request) {
        log.debug("Unsupported exchange requested: {} | path: {}", ex.getMessage(), request.getRequestURI());
    ...
    /** Builds a consistent error response DTO for all exception handlers. */
