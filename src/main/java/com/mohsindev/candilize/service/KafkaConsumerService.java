...
/**
 * Kafka consumer for the get-price topic. Receives KafkaPriceRequest messages (from scheduler, download
 * controller, or cache refresh), delegates to CandleDownloadService (which uses CandleDataProviderFactory
 * and CandleDataPersistenceService). Failures are logged per message so one bad request does not stop others.
 */
@Slf4j
@Service
